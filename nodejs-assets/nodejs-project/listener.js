...
async function run() {
  ...
  const cid = new CID(listenerNode.peerInfo.id.toB58String());
  await listenerNode.start();
  await listenerNode.contentRouting.provide(cid);
  console.log('Providing for :' + listenerNode.peerInfo.id.toB58String());
  //  const listenerId = await PeerId.createFromJSON(require('./id-l'));
  //
  //  // Listener libp2p node
  //  const listenerPeerInfo = new PeerInfo(listenerId);
